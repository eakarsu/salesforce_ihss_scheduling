...
export default function FormModal({ columns, color, onClose, onSubmit, title }) {
    ...
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div style={styles.header(color)}>
          <span style={styles.title(color)}>{title || 'Add New Item'}</span>
        ...
        <div style={styles.actions}>
          <button style={styles.btn('#475569')} onClick={onClose}>Cancel</button>
          <button style={styles.btn('#3b82f6')} onClick={handleSubmit}>Create</button>
        </div>
      </div>
    </div>
  );
